...
static const double LOCATION_RANGE = 10000.0; // Map size in km
...
static const double CROSSOVER_POINT =
    0.5; // Child inherit 50% of father genes and 50% of mother genes
...
random_device rd{};
mt19937 gen{rd()};
uniform_real_distribution<double> distributionLocation{-LOCATION_RANGE,
                                                       LOCATION_RANGE};
...
int main() {
  ...
  out << "Done!\nExecution time: "
      << chrono::duration_cast<chrono::seconds>(done - started).count() << "s"
      << endl;
  cout << "Done!\nExecution time: "
       << chrono::duration_cast<chrono::seconds>(done - started).count() << "s"
       << endl;
...
const double Gene::calcDistance(const Gene &city) const {
  return sqrt((this->x - city.x) * (this->x - city.x) +
              (this->y - city.y) * (this->y - city.y));
}

const string Gene::toString() const {
  return "{" + to_string(this->id) + ": (" + to_string(this->x) + ", " +
         to_string(this->y) + ")}";
}
...
const string Chromosome::toString() const {
  ...
  output += to_string(this->DNA[this->DNA.size() - 1].getId()) + "] -> " +
            to_string(this->fitness);
  return output;
}

const vector<Gene> &Chromosome::getDNA() const { return this->DNA; }

const double Chromosome::getFitness() const { return this->fitness; }
...
void printIndividualWithBestFitness(const int generation) {
  out << "\nGeneration " << generation << " with " << POPULATION_SIZE
      << " individuals. Average fitness: " << avgFitness
...
void crossover() {
  ...
  for (int i = 0; i < selectionSize / 2; ++i) {
    ...
    vector<Gene> child{father.begin(),
                       father.begin() + (int)(father.size() * CROSSOVER_POINT)};
    for (auto geneOfMother : mother) {
      if (find_if(child.begin(), child.end(), [&](const Gene &gene) -> bool {
